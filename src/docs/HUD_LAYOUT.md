...
> **等比缩放（2026-07-24）：** 舞台 `ResizeObserver` 写入 `--yp-hud-scale = clamp(0.55, 舞台高/1080, 1)`，并在 **同一元素上重声明** HUD token（避免 `:root` 里 `calc(scale)` 算死后继承、矮窗不缩小）。
...
| Top HUD | 玩家五项 + 任期进度 | 资源条、小型进度框 | 大面板、长文、主入口 |
